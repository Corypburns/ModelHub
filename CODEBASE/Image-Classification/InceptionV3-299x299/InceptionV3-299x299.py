...
match host:
    case "DESKTOP-FI8GT7F":
        ...
        TEST_IMAGE_BASE_PATH = Path(r"E:\Code\Python\DATASETS\HAMM")
    case "CoryPC":
...
TEST_IMAGE_PATH = TEST_IMAGE_BASE_PATH / "Master_images"
MODEL_PATH = BASE_PATH / "MODELBASE" / "Image-Classification" / "InceptionV3-299x299" / "inceptionv3.tflite"
LOG_DIR = BASE_PATH / "OUTPUTS" / "Image-Classification" / "InceptionV3-299x299"
LABEL_MAP = BASE_PATH / "LABELMAPS" / "Image-Classification" / "HAM10000_metadata.csv"
label_file = pd.read_csv(str(LABEL_MAP))
important_labels = label_file['dx'].tolist()
DATE_TIME = dt.now().strftime("%y%m%d_%H%M%S")
FILE_NAME = f"log_IV3_299x299(float)_{DATE_TIME}.csv"
...
def image_processing_inference(interpreter, img_path, labels=None, mode="CPU1"):
    ...
    # Create a mapping from image file stem to true label
    label_map_df = pd.read_csv(str(LABEL_MAP))
    # Assuming the CSV has columns: 'image_id' and 'dx' (you may need to adjust names)
    label_map = dict(zip(label_map_df['image_id'], label_map_df['dx']))
    ...
    for img_path in TEST_IMAGE_PATH.glob("*.jpg"):
        ...
        # Get true label for current image from label_map
        image_id = img_path.stem  # filename without extension
        true_label = label_map.get(image_id, "Unknown")
        
        confidence = output[predicted_index]
        
        print(f"Image: {img_path.name} | Prediction: {predicted_label} - {(confidence) * 100:.2f}% | Actual: {true_label} | ({(delay_end - delay_start) * 1000:.2f} ms.)")
        cv2.imshow(img_path.name, raw_img)
        cv2.waitKey(3000)
...
def menu():
    ...
    match choice:
        case 1:
            ...
            image_processing_inference(interpreter, TEST_IMAGE_PATH, important_labels, mode)
        case 2:
            mode="CPU4"
